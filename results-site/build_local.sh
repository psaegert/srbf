...
# private/results_v2_private.js (both paths are git-ignored). The public index.html is not touched and contains
# no reference to either file. Serve locally:  cd results-site && python3 -m http.server 8765  -> /index.local.html
set -eu
cd "$(dirname "$0")"
[ -f private/results_v2_private.js ] || { echo "no private/results_v2_private.js (write it with scripts/site_export_v2.py --private ... --private-out results-site/private/results_v2_private.js)"; exit 1; }
for f in private/results_v2_private.js index.local.html; do git check-ignore -q "$f" || { echo "REFUSED: $f is not git-ignored"; exit 1; }; done
python3 - <<'PY'
s = open("index.html").read()
tag = '<script src="private/results_v2_private.js" charset="utf-8"></script>\n  <script src="explorer_v2.js"'
