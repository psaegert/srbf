...
| File | Purpose |
|------|---------|
| `index.html` | The page shell (header, hero, explorer mount, the "How to read the results" section (`#about`), the "Paired comparisons" (`#paired`) and "Ranks" (`#ranks`) sections, the "Metrics" reference with KaTeX-rendered formulas, privacy note, footer). Written for a first-time reader: every term is defined where it first appears, and the explorer's short hints link here for the longer explanations. |
