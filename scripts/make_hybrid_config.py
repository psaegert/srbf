...
Usage:
  make_hybrid_config.py --model-path DIR --model-name NAME --pysr-python PY \
      ...
      [--landing-tolerance 0.01] [--pysr-overhead 4.0] [--pricing-reserve 0.2]
Both stages run by the clock (see srbf/hybrid_adapter.py); the three knobs are the landing tolerance
of the generation stage and the initial estimates of the two costs PySR's share pays besides the search.
"""
...
def main() -> int:
        ...
        experiments[cat] = {
            ...
            "model_adapter": {
                ...
                "hybrid": {
                    ...
                    "landing_tolerance": a.landing_tolerance, "pysr_overhead_s": a.pysr_overhead,
                    "pricing_reserve_s": a.pricing_reserve,
    ...
    header = (f"# Flash-ANSR seeding + PySR at a fixed budget of {a.budget:g} s per problem: ratio r of the budget to PySR\n"
              f"# (`--sweep-filter ratio=<r>`), K = {a.k_seeds} seeds, both stages by the clock. Generated by scripts/make_hybrid_config.py.\n")
