...
METRICS = [
    ...
    ("numeric_recovery_val", "Numeric Recovery, Validation (vNRR)", "vNRR", "Numeric Recovery", "rate", True, "main", "pct", None,
     "Share of problems whose prediction reproduces the validation targets to float32 precision: FVU on the validation split at or below 2^-23. A failed prediction is a miss."),
    ("numeric_recovery_fit", "Numeric Recovery, Support (fNRR)", "fNRR", "Numeric Recovery", "rate", True, "more", "pct", None,
    ...
    ("symbolic_recovery_mask_none", "Symbolic Recovery: Structure + All Numbers (SRRa)", "SRRa", "Symbolic Recovery", "rate", True, "more", "pct", None,
     "Symbolic recovery with nothing masked: the structure and its exponents are the ground truth's, and the fitted constants reproduce it to float32 precision on the validation points. The expression itself was found."),
    ("skeleton_match_raw", "Raw Symbolic Recovery (SRRr)", "SRRr", "Symbolic Recovery", "rate", True, "more", "pct", None,
     "Symbolic recovery without simplification: the predicted skeleton equals the ground-truth skeleton token for token as both were written, every number masked. A re-ordered sum or x*x for x^2 is a miss here. This is the 2026-07 site's symbolic recovery, kept for comparability."),
    ...
    ("mdl_ratio", "MDL Ratio (Prediction / Ground Truth)", "MDL Ratio", "Size Compared to the Ground Truth", "cont", None, "main", "ratio", (-4.0, 4.0, "log2"),
    ...
    ("expr_length_ratio", "Token Count Ratio (Prediction / Ground Truth)", "Token Ratio", "Size Compared to the Ground Truth", "cont", None, "main", "ratio", (-4.0, 4.0, "log2"),
    ...
    ("expr_length_ratio_abserr", "Token Count Mismatch (|log2 Ratio|)", "|log2 Ratio|", "Size Compared to the Ground Truth", "cont", False, "more", "num2", (0.0, 4.0, None),
     "Absolute log2 of the token count ratio: 0 when the counts match, 1 at twice or half the length."),
    ("n_constants_ratio", "Constant Count Ratio (Prediction / Ground Truth)", "Constants Ratio", "Size Compared to the Ground Truth", "cont", None, "more", "ratio", (-4.0, 4.0, "log2"),
    ...
    ("n_constants_delta", "Constant Count Difference (Prediction - Ground Truth)", "Constants Diff.", "Size Compared to the Ground Truth", "cont", False, "more", "num1", (-16.0, 16.0, None),
     "Predicted minus true constant count."),
    ("total_nestedness_delta", "Function Nesting Difference (Prediction - Ground Truth)", "Nesting Diff.", "Size Compared to the Ground Truth", "cont", False, "more", "num1", (-8.0, 8.0, None),
    ...
    ("edit_distance", "Levenshtein Edit Distance (Tokens)", "Levenshtein", "Similarity to the Ground Truth", "cont", False, "more", "num1", (0.0, 64.0, None),
    ...
    ("edit_distance_norm", "Levenshtein Edit Distance (Tokens), Normalized", "Levenshtein Norm.", "Similarity to the Ground Truth", "cont", False, "more", "num3", (0.0, 1.0, None),
     "Levenshtein distance between the prefix token sequences over the longer length, in [0, 1]."),
    ("zss_edit_distance", "Tree Edit Distance (Zhang-Shasha)", "Tree Edit Dist.", "Similarity to the Ground Truth", "cont", False, "more", "num1", (0.0, 128.0, None),
    ...
    ("predicted_mdl", "MDL of the Prediction (Bits)", "Prediction MDL", "Expression Properties", "cont", False, "more", "num1", (0.0, 256.0, None),
     "Description length of the predicted expression in bits (SimpliPy mu, f64 canon)."),
    ("ground_truth_mdl", "MDL of the Ground Truth (Bits)", "GT MDL", "Expression Properties", "cont", None, "more", "num1", (0.0, 256.0, None),
