...
def main() -> None:
    ...
    for model, repo, prior in arms:
        ...
        head = (f"# {what} on every srbf catalog: recovery as a function of the sampling budget, 1 to 16,384\n"
                f"# draws. One experiment per catalog (`--experiment <catalog>`), one rung per candidate count\n"
                f"# (`--sweep-filter ladder=<draws>`), ONE draw per expression per pass; run the config again under\n"
