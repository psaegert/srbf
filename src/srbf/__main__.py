"""srbf command-line interface: the ``evaluate-run`` subcommand carved from flash-ansr.

flash-ansr keeps the rest of its CLI (train / benchmark / import-data / install / ...); only
``evaluate-run`` is evaluation-bound and lives here. The eval imports are ``srbf.eval.*``; the
