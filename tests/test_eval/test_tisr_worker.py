...
def _report(members, values, stop="reached maximum number of generations", generations=8):
    """SrbfTiSR.run's report: members in TiSR's order, each its tree's tokens and TiSR's measures."""
    return {"members": [{"tokens": tokens, "measures": {m: float(i + 1) for i, m in enumerate(MEASURES)}}
                        for tokens in members],
            "values": list(values), "seconds": 1.25, "stop": stop, "generations": generations,
            "progress": [[1, generations], [0.01, 1.2]]}
...
class _Julia:
    ...
    def __init__(self):
        ...
        # the answer 2.5*v1*v2 + 1.0, as TiSR's tree: + (1.0) (* (2.5) (* v1 v2)); binops are + - * / ^ rootn
        self.report = lambda X: _report(
            [["b1", "p1.0", "b3", "p2.5", "b3", "v1", "v2"], ["b3", "v1", "v2"]],
            2.5 * X[:, 0] * X[:, 1] + 1.0)
...
def test_tisr_runs_at_its_defaults_with_the_benchmark_operators_the_budget_and_one_thread(julia):
    ...
    assert ("general = general_params(; n_gens = n_gens, t_lim = t_lim, multithreading = false, print_progress = false, "
            "plot_hall_of_fame = false, print_hall_of_fame = false)") in source
...
def test_the_answer_is_the_first_member_in_tisrs_order_written_in_the_benchmark_syntax(julia):
    X, y = _data()
    out = w.fit(X, y, x_val=[], variables=["a", "b"], meta={}, options={}, state=w.load({"warmup": False}))
    assert out["expression"] == "(1.0 + (2.5*(a*b)))"
    assert "y_pred" not in out                       # srbf evaluates the string
    hof = out["extra"]["hall_of_fame"]
    assert [m["expression"] for m in hof] == ["(1.0 + (2.5*(a*b)))", "(a*b)"]
    assert set(hof[0]) == set(MEASURES) | {"expression"}
    assert out["extra"]["string_deviation"] < 1e-12
...
def test_a_string_that_does_not_compute_tisrs_values_shows_in_the_deviation(julia):
    ...
    out = w.fit(X, y, x_val=[], variables=["a", "b"], meta={}, options={}, state=w.load({"warmup": False}))
    assert out["extra"]["string_deviation"] > 0.1
...
def test_an_answer_that_cannot_be_written_is_a_failed_problem(julia):
    X, y = _data()
    julia.report = lambda X: _report([["pNaN"], ["b3", "v1", "v2"]], np.full(X.shape[0], np.nan))
    out = w.fit(X, y, x_val=[], variables=["a", "b"], meta={}, options={}, state=w.load({"warmup": False}))
    assert "non-finite" in out["error"] and out["extra"]["hall_of_fame"][1]["expression"] == "(a*b)"
...
def test_a_side_experiment_replaces_settings_and_sets_per_problem_values_at_run_time(julia):
    config = {"binops": ["+", "-", "*", "/", "^"], "unaops": ["neg", "sqrt", "pow2"],
              "data_split": {"parts": "[0.5, 0.0, 0.5]"}, "general": {"callback": "(h, p, g, t, d, o) -> false"},
    ...
    assert "callback = (h, p, g, t, d, o) -> false" in source
    X, y = _data()
...
print("REPORT " + json.dumps({"keys": sorted(str(k) for k in report.keys()), "member": sorted(str(k) for k in member.keys()),
      ...
      "n_values": len(list(report["values"]))}))
out = w.fit(X, y, x_val=[], variables=["a", "b"], meta={}, options={}, state=state)
print("FIT " + json.dumps({"expression": out["expression"], "deviation": out["extra"]["string_deviation"]}))
"""
...
def test_the_stand_in_reports_what_the_real_environment_reports(tmp_path):
    ...
    assert lines["FIT"]["deviation"] is not None and lines["FIT"]["deviation"] < 1e-9
