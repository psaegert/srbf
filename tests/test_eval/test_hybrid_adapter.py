"""The hybrid arm's adapter is thin: it hands each problem to flash-ansr's hybrid regressor and records
the answer like every adapter (the method itself is tested in flash-ansr, flash_ansr.hybrid)."""
...
class _FakeRegressor:
    ...
    def solve(self, X, y, *, X_val=None, variables=None, problem_id=None, complexity=None):
...
def test_builder_runs_against_the_real_flash_adapter(monkeypatch):
    ...
    # stayed green. Build through the real class and a stand-in flash_ansr.hybrid that records its config.
    import sys
    import types
    ...
    monkeypatch.setitem(sys.modules, "flash_ansr.hybrid",
