...
def test_nesymres_is_handed_every_column_whatever_the_law_uses(monkeypatch):
    """Which columns a law uses is part of the answer. The law below uses x3 and x4; a model that takes three
    inputs is handed the first three columns, as for any other problem, and never the two the law names."""
    monkeypatch.setattr(model_adapters, "_HAVE_NESYMRES", True)
    ...
    adapter = model_adapters.NeSymReSAdapter(
        ...
        simplipy_engine=_DummyEngine(),
        remove_padding=True,             # accepted, ignored
    )
    ...
    assert "variable_names" not in result.values or result.values["variable_names"] != ["x3", "x4"]
