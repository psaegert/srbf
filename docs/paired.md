...
Every method is evaluated on the same expressions. A comparison of two methods should use that: a hard
expression is hard for both, so the uncertainty of each method's average is dominated by the spread
between expressions, and that spread cancels in the difference. `srbf.reporting` compares methods expression by
expression,
...
overlap while one method is better on expression after expression, so compare methods with a paired report and
...
same benchmark, one per method. The report pairs the expressions by `benchmark_eq_id`, averages the problems of an expression, and
bootstraps the expressions. It contains:

| key | meaning |
|---|---|
| `delta_mean`, `ci_lower`, `ci_upper` | the mean of \(\Delta\), A minus B, with its percentile interval over expressions |
| `p_value` | two-sided, from the same bootstrap |
| `delta_median`, `median_ci_lower`, `median_ci_upper` | the same for the median |
| `win_rate` | the numbers of expressions where A is better, where B is better, and where they tie: `a_better`, `b_better`, `tied` |
| `prob_superiority` | the probability that A beats B on a random expression, a tie counting half |
| `wilcoxon` | a signed-rank test that keeps the zeros (`zero_method="pratt"`): on a rate most expressions tie, and dropping the ties would change the test |
| `mde_80` | the smallest true difference this comparison would detect 80 % of the time |
| `n_pairs`, `n_only_a`, `n_only_b` | the expressions both methods have a value for, and those only one has |
| `variance_decomposition` | how much of the variance of \(\Delta\) is between expressions and how much comes from the problems drawn within an expression |

An exact fit has `log10_fvu_val` \(-\infty\), which makes a mean of differences infinite, and one
diverging prediction decides a mean of `r2_val`. For these columns read `delta_median` and the ranks.

### What is paired

A success metric is defined for every expression, so it is paired over all of them and a method that
fails on an expression has missed it. An analysis metric exists only where there is a prediction, so it is
paired over the expressions where both methods have one, and `n_only_a` and `n_only_b` say how many were
left out. `worst_rank=True` adds rank statistics over all expressions in which a missing value is placed
last; it is off by default, because a rank over all expressions mixes how often a method returns a
prediction into how good its predictions are.

### Pairing is checked

Expressions are joined by id and never by row order. The provenance of the two snapshots (`__meta__`,
...
Two runs on a named catalog share the expressions and not the points
...
With several problems per expression (`problems_per_expression` above 1), the noise of the benchmark
...
values_a = draw_values(scored_a, "numeric_recovery_val")     # {expression id: the values of its problems}
...
`self_noise` compares a method with itself across random halves of each expression's problems; `pair_margin`
...
At equal time each method is brought to \(t\) per expression, by linear interpolation in log time between
its two neighboring rungs; an expression counts only where it has values at both neighbors on both sides.
...
values = {name: {expression: v.mean() for expression, v in draw_values(s, "log10_fvu_val").items()} for name, s in scored.items()}
ranks = rank_league(values, higher_is_better=False)
print(ranks["mean_ranks"], ranks["friedman_p"], ranks["cd"], ranks["cliques"])
```

Within each expression the methods are ranked, 1 being best and ties sharing the mean of their places; a
method without a value for an expression ranks last on it. Every expression hands out the same places, so easy and
hard expressions count alike. The Friedman test, corrected for ties, asks whether the mean ranks differ at
...
for \(k\) methods over \(K\) expressions, is the smallest gap between two mean ranks that counts as real
...
For a quick contrast of two rates on the same expressions, `srbf.metrics` has the exact tools:
...
problems in the same order: two complete runs of one catalog with one problem per expression do.

## Why expressions are the unit

The expressions are the exchangeable unit of a benchmark; the problems drawn for one expression are repeated
measurements of it. Averaging them per expression and bootstrapping the expressions is the cluster bootstrap for
the mean, and no further variance correction applies, because no data is shared between the
resampled units. `hierarchical=True` additionally resamples the problems within each expression for the
rank statistics.
