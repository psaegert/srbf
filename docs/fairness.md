...
- **TiSR:** `upstream_default`: its own defaults at the commit its author's benchmark paper ran. That paper's
  protocol, with its operators and forbidden nestings and a complexity cap per problem taken from the ground truth,
  runs as a `harness_tuned` config under `configs/evaluation/panels/`, a check against the paper's numbers.
